...
{
    public static class PBEDataUtils
    {
        ...
        private static ushort CalcHP(PBESpecies species, byte evs, byte ivs, byte level)
        {
            return (ushort)(species == PBESpecies.Shedinja ? 1 : ((((2 * PBEPokemonData.GetData(species).BaseStats[0]) + ivs + (evs / 4)) * level / 100) + level + 10));
        }
        private static ushort CalcOtherStat(PBEStat stat, PBESpecies species, sbyte statRelationship, byte evs, byte ivs, byte level, PBESettings settings)
        {
            double natureMultiplier = 1 + (statRelationship * settings.NatureStatBoost);
            return (ushort)(((((2 * PBEPokemonData.GetData(species).BaseStats[(int)stat]) + ivs + (evs / 4)) * level / 100) + 5) * natureMultiplier);
        }
        /// <summary>Calculates a species's stats based on its level, IVs, EVs, and nature.</summary>
        /// <exception cref="ArgumentNullException">Thrown when <paramref name="settings"/> is null.</exception>
        /// <exception cref="ArgumentOutOfRangeException">Thrown when <paramref name="species"/> or <paramref name="stat"/> is invalid.</exception>
        public static ushort CalculateStat(PBEStat stat, PBESpecies species, PBENature nature, byte evs, byte ivs, byte level, PBESettings settings)
        {
            ...
            switch (stat)
            {
                case PBEStat.HP:
                {
                    return CalcHP(species, evs, ivs, level);
                }
                case PBEStat.Attack:
                ...
                case PBEStat.Speed:
                {
                    return CalcOtherStat(stat, species, nature.GetRelationshipToStat(stat), evs, ivs, level, settings);
                }
        ...
        public static void GetStatRange(PBEStat stat, PBESpecies species, byte level, PBESettings settings, out ushort low, out ushort high)
        {
            ...
            switch (stat)
            {
                case PBEStat.HP:
                {
                    low = CalcHP(species, 0, 0, level);
                    high = CalcHP(species, byte.MaxValue, settings.MaxIVs, level);
                    break;
                }
                case PBEStat.Attack:
                case PBEStat.Defense:
                ...
                case PBEStat.Speed:
                {
                    low = CalcOtherStat(stat, species, -1, 0, 0, level, settings);
                    high = CalcOtherStat(stat, species, +1, byte.MaxValue, settings.MaxIVs, level, settings);
                    break;
                }
                default: throw new ArgumentOutOfRangeException(nameof(stat));
            }
